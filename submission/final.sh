...
BASE_DECODED=$(bitcoin-tx -json -chain=regtest "$BASE_TX")

TXID=$(echo "$BASE_DECODED" | jq -r '.txid')
...
# WRITE YOUR SOLUTION BELOW:
NUM_INPUTS=$(echo "$BASE_DECODED" | jq -r '.vin | length')
check_cmd "Input counting" "NUM_INPUTS" "$NUM_INPUTS"

NUM_OUTPUTS=$(echo "$BASE_DECODED" | jq -r '.vout | length')
...
RAW_TX=$(bitcoin-tx -create -chain=regtest nversion=2 "in=${UTXO_TXID}:${UTXO_VOUT_INDEX}:1" "outaddr=${PAYMENT_BTC}:${PAYMENT_ADDRESS}" "outaddr=${CHANGE_BTC}:${CHANGE_ADDRESS}")
...
# WRITE YOUR SOLUTION BELOW:
DECODED_TX=$(bitcoin-tx -json -chain=regtest "$RAW_TX")
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" '.vout[] | select(.scriptPubKey.address == $addr) | .value' | awk '{printf "%.8f", $1}')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address == $addr) | .value' | awk '{printf "%.8f", $1}')
...
SIMPLE_RAW_TX=$(bitcoin-tx -create -chain=regtest nversion=2 "in=${TXID}:0:4294967293" "outaddr=0.00010000:${TEST_ADDRESS}")
...
PARENT_TXID=$(bitcoin-tx -txid -chain=regtest "$RAW_TX")
...
CHILD_RAW_TX=$(bitcoin-tx -create -chain=regtest nversion=2 "in=${PARENT_TXID}:${CHANGE_OUTPUT_INDEX}:1" "outaddr=${CHILD_SEND_BTC}:${CHILD_RECIPIENT}")
...
SECONDARY_DECODED=$(bitcoin-tx -json -chain=regtest "$SECONDARY_TX")
...
TIMELOCK_TX=$(bitcoin-tx -create -chain=regtest nversion=2 "in=${SECONDARY_TXID}:1:10" "outaddr=${TIMELOCK_BTC}:${TIMELOCK_ADDRESS}")
